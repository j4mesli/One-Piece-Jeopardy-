...
function Game() {
  ...
  const deleteTests = async () => {
    const res = await fetch("http://localhost:3000/deleteTest", {
      method: "DELETE",
      headers: {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch("http://localhost:3000/evaluateTest", {
      method: "POST",
      headers: headers
    });
    const data = await res.json();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const res = await fetch("http://localhost:3000/fetchTestQuestions", {
        method: "GET",
        headers: headers
      });
      const data = await res.json();
  ...
  return (
    <div className="game">
      <h1>Game</h1>
      { showGame ? (
        <form className="game-form" action="http://localhost:3000" onSubmit={handleSubmit}>
        <ul>
