...
function Game() {
  ...
  const deleteTests = async () => {
    const res = await fetch("one-piece-jeopardy-backend-d2ca7583addf.herokuapp.com/deleteTest", {
      method: "DELETE",
      headers: {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch("one-piece-jeopardy-backend-d2ca7583addf.herokuapp.com/evaluateTest", {
      method: "POST",
      headers: headers
    });
    const data = await res.json();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const res = await fetch("one-piece-jeopardy-backend-d2ca7583addf.herokuapp.com/fetchTestQuestions", {
        method: "GET",
        headers: headers
      });
      const data = await res.json();
  ...
  return (
    <div className="game">
      <h1>Game</h1>
      { showGame ? (
        <form className="game-form" action="one-piece-jeopardy-backend-d2ca7583addf.herokuapp.com" onSubmit={handleSubmit}>
        <ul>
