...
function Login() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('https://-jeopardy-backend-d2ca7583addf.herokuapp.com/login', {
        method: 'POST',
        headers: headers,
      });
  ...
  return (
    <div className="login">
        <form action="https://-jeopardy-backend-d2ca7583addf.herokuapp.com" method="GET" className="loginForm" onSubmit={ handleSubmit }>
            <h1>Login</h1>
